...
pub fn init_db() {
...

    {
        ...
        let column_info: Vec<String> = stmt.query_map([], |row| row.get::<_, String>(1))
        ...
        for column in columns {
            ...
            if !column_info.contains(&column_name.to_string()) {
                let add_column_query = format!("ALTER TABLE clipboard_entries ADD COLUMN {}", column);
    ...
    DATABASE_CONNECTION.set(Mutex::new(conn)).expect("Failed to set database connection");
}


// Function to get the last item copied of a specific type
pub fn get_last_item_copied(item_type: &str) -> Option<ClipboardEntry> {
    let conn = DATABASE_CONNECTION.get().expect("DB non initialisée").lock().unwrap();
...
pub fn insert_clipboard_entry(entry_type: &str, content: &str, pinned: i32) -> i64 {
    ...
    if res == 0 {
        println!("Failed to insert clipboard entry");
    } else {
        println!("Inserted clipboard entry: {} - {}", entry_type, content);
    }
...
pub fn get_all_ids() -> Vec<i64> {
    ...
    let conn = DATABASE_CONNECTION.get().expect("DB not initialized").lock().unwrap();
    let mut stmt = conn.prepare("SELECT id FROM clipboard_entries ORDER BY added_at DESC").unwrap();
...
pub fn get_truncated_item_by_id(id: i64) -> Option<ClipboardEntry> {
    ...
    if entry.entry_type != "image" && content.len() > configuration.max_displayed_characters as usize {
        content = content
            .chars()
...
pub fn delete_item_by_id(id: i64) {
    // Get the database connection
    let conn = DATABASE_CONNECTION.get().expect("DB non initialisée").lock().unwrap();
    let res = conn.execute("DELETE FROM clipboard_entries WHERE id = ?1", [id]).unwrap();
    if res == 0 {
        println!("Failed to delete clipboard entry with ID: {}", id);
    } else {
        println!("Deleted clipboard entry with ID: {}", id);
    }
}
...
pub fn delete_all_items() -> i64 {
    ...
    if res == 0 {
        ...
        println!("Deleted all clipboard entries");
    }
...
pub fn pin_item_by_id(id: i64) {
    // Get the database connection
    let conn = DATABASE_CONNECTION.get().expect("DB non initialisée").lock().unwrap();
    let res = conn.execute("UPDATE clipboard_entries SET pinned = 1 WHERE id = ?1", [id]).unwrap();
    if res == 0 {
        println!("Failed to pin clipboard entry with ID: {}", id);
    } else {
        println!("Pinned clipboard entry with ID: {}", id);
    }
}
...
pub fn unpin_item_by_id(id: i64) {
    // Get the database connection
    let conn = DATABASE_CONNECTION.get().expect("DB non initialisée").lock().unwrap();
    let res = conn.execute("UPDATE clipboard_entries SET pinned = 0 WHERE id = ?1", [id]).unwrap();
    if res == 0 {
        println!("Failed to unpin clipboard entry with ID: {}", id);
    } else {
        println!("Unpinned clipboard entry with ID: {}", id);
    }
}
...
pub fn unpin_all_items() -> i64 {
    // Get the database connection
    let conn = DATABASE_CONNECTION.get().expect("DB non initialisée").lock().unwrap();
    let res = conn.execute("UPDATE clipboard_entries SET pinned = 0", []).unwrap();
    if res == 0 {
        println!("Failed to unpin all clipboard entries");
    } else {
        println!("Unpinned all clipboard entries");
    }
...
pub fn set_forced_language(id: i64, language: &str) {
    ...
    let res = conn.execute("UPDATE clipboard_entries SET forced_language = ?1 WHERE id = ?2", [language,  &id]).unwrap();
    if res == 0 {
        println!("Failed to set forced language for clipboard entry with ID: {}", id);
    } else {
        println!("Set forced language for clipboard entry with ID: {}", id);
    }
}